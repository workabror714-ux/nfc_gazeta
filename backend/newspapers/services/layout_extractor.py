...
def render_image_block(
    ...
    if isinstance(
        ...
        try:
            ...
            logger.debug(
                "Embedded image conversion failed; "
    ...
    if (
        clip_rect.is_empty
        or clip_rect.is_infinite
    ):
        raise ValueError(
            "Rasm hududi noto‘g‘ri."
        )
...
def extract_page_layout(
    ...
    for reading_order, block in enumerate(
        ...
        if (
            ...
            or image_height < MIN_IMAGE_HEIGHT
            or image_area < MIN_IMAGE_AREA
        ):
            skipped_image_count += 1
            continue
        ...
        if checksum in known_image_checksums:
            skipped_image_count += 1
            continue

        known_image_checksums.add(
            checksum
        )
